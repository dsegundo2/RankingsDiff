...
def get_text_color(a, b, pos):
    ...
    if perc_diff > 0.20 and b < 200 and diff > 12 or perc_diff > 0.50:
        return COLOR_VERY_GOOD
    if (
        perc_diff < -0.12
        and 10 < b < 200
        and diff < -15
        and not str(pos).startswith("QB")
    ):
        return COLOR_VERY_BAD
    if perc_diff > 0.15 and a < 200 and diff > 4:
        return COLOR_GOOD
    if perc_diff < -0.15 and b > 10 and diff < -4 or perc_diff < -0.60:
        return COLOR_BAD
    return COLOR_NEUTRAL
