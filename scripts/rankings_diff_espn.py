...
def format_merged_list(merged_list):
    ...
    merged_list[UNDERDOG_RANKING_COLUMN] = merged_list[UNDERDOG_RANKING_COLUMN].astype('Int64')
    merged_list[ESPN_RANKING_COLUMN] = merged_list[ESPN_RANKING_COLUMN].astype('Int64')

    # Only keep desired columns
    merged_list = merged_list[[OUTPUT_POSITION_COLUMN, ESPN_RANKING_COLUMN, UNDERDOG_RANKING_COLUMN, UNDERDOG_PLAYER_COLUMN, 'Team', ESPN_AUCTION_VALUE_COLUMN_ORIGINAL, UNDERDOG_AUCTION_VALUE_COLUMN]].sort_values(by=ESPN_RANKING_COLUMN)
    # Renaming columns
...
def create_output_files(df):
    ...
    styled_df = df.style.apply(lambda col: highlight_cell_values(
        col, df[ESPN_RANKING_COLUMN], df[OUTPUT_POSITION_COLUMN], get_text_color), subset=[UNDERDOG_RANKING_COLUMN]).apply(
            lambda col: highlight_cell_values(col, df[ESPN_AUCTION_VALUE_COLUMN], df[OUTPUT_POSITION_COLUMN], get_text_color_auction),
            subset=[UNDERDOG_AUCTION_VALUE_COLUMN]).set_properties(**{'text-align': 'center'})

    styled_df = styled_df.map(highlight_positions, subset=[OUTPUT_POSITION_COLUMN])
    styled_df = styled_df.format({UNDERDOG_AUCTION_VALUE_COLUMN: '${:,.0f}'}).format({ESPN_AUCTION_VALUE_COLUMN: '${:,.0f}'})
    styled_df.to_excel(OUTPUT_EXCEL, engine='openpyxl', index=False)

    # Add dollar signs to auction values
    df[ESPN_AUCTION_VALUE_COLUMN] = df[ESPN_AUCTION_VALUE_COLUMN].apply(lambda x: f"${int(x):,}")
    df[UNDERDOG_AUCTION_VALUE_COLUMN] = df[UNDERDOG_AUCTION_VALUE_COLUMN].apply(lambda x: f"${int(x):,}")
...
def add_columns(df):
    ...
    lookup_map = df.set_index(ESPN_RANKING_COLUMN)[ESPN_AUCTION_VALUE_COLUMN_ORIGINAL].to_dict()
...
merged_df = pd.merge(ud_dataframe, espn_dataframe,
                        left_on=ud_dataframe[UNDERDOG_PLAYER_COLUMN].str.lower(),
                        right_on=espn_dataframe[ESPN_PLAYER_COLUMN].str.lower(), how='inner')
