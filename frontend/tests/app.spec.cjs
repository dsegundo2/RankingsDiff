...
test('ESPN defaults to snake and exposes an auction format switch', async ({ page }) => {
  ...
  await expect(page.locator('tbody tr[data-ranking-id]').filter({ hasText: 'Kenneth Walker' }).first().locator('.diff-cell')).toHaveText('12')
  await expect(page.locator('tbody tr[data-ranking-id]').filter({ hasText: 'Jordan Love' }).first().locator('.diff-cell')).toHaveText('77')
  await page.getByRole('button', { name: /Settings/ }).click()
  await openCurrentSheet(page)
  ...
  await expect(page.locator('tbody tr[data-ranking-id]').filter({ hasText: 'Kenneth Walker' }).first().locator('.diff-cell')).toHaveText('+$17')
  await expect(page.locator('tbody tr[data-ranking-id]').filter({ hasText: 'Jordan Love' }).first().locator('.diff-cell')).toHaveText('+$1')
  await page.getByRole('button', { name: /Settings/ }).click()
  await openCurrentSheet(page)
