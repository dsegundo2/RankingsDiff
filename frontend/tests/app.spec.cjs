...
test('screenshot auction rankings load as an ordered base sheet', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Close settings' }).click()
  await page.getByRole('textbox', { name: 'Search players' }).fill('Jahmyr Gibbs')
  await expect(page.locator('tr[data-ranking-id]').first()).toContainText('$56')
...
test('auction player inspector shows history and supports close and draft actions', async ({ page }) => {
  ...
  await expect(inspector.locator('.auction-inspector__brand-mark')).toHaveCount(1)
  ...
  await expect(inspector.locator('tfoot').first().locator('td').nth(1)).not.toHaveText('—')
  await expect(inspector.locator('tfoot tr.is-average')).toHaveCount(2)
