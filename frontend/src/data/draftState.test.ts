...
describe('draft state', () => {
  ...
  it('round-trips target and drafted selections', () => {
    writeDraftState('draft', { targets: ['one'], drafted: ['two'], picks: {}, draftSlot: 2 })
    expect(readDraftState('draft')).toEqual({ targets: ['one'], drafted: ['two'], picks: {}, draftSlot: 2 })
  })

  it('recovers from invalid cached data', () => {
    localStorage.setItem('draft', '{broken')
    expect(readDraftState('draft')).toEqual({ targets: [], drafted: [], picks: {}, draftSlot: 2 })
  })
  ...
  it('exports and restores player-keyed draft files', () => {
    ...
    expect(parseDraftFile(JSON.stringify(file))).toMatchObject({
      ...
      season: 2026,
      source: 'espn',
      players: { targets: ["ja'marr chase|cin"], drafted: ['jahmyr gibbs|det'], draftSlot: 7 },
  ...
  it('round-trips a portable cross-device draft link', () => {
    const url = createDraftShareUrl(2026, 'espn', { targets: ["ja'marr chase|cin"], drafted: ['jahmyr gibbs|det'], picks: { 'jahmyr gibbs|det': 1 }, draftSlot: 9 })
    const shared = readDraftShare(new URL(url).searchParams.get('draft'))
    expect(shared).toEqual({ season: 2026, source: 'espn', targets: ["ja'marr chase|cin"], drafted: ['jahmyr gibbs|det'], picks: { 'jahmyr gibbs|det': 1 }, draftSlot: 9 })
  })
})
