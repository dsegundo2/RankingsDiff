...
import { enrichDraftRows, overallSlotHistory, slotHistory, slotSummaries, tierSummaries, type HistoricalYear } from '../data/draftAnalytics'
...
export function AuctionPlayerInspector({ row, allRows, draftedCount, dragOffset, rowsBySeason, season, teams, favorited, onFavorite, onClose, onDraft, onAdd, onNavigate, onDragOffsetChange }: Props) {
  ...
  const overallSlots = overallHistory.length ? { average_expected: overallHistory.reduce((sum, item) => sum + item.expected, 0) / overallHistory.length, average_paid: overallHistory.reduce((sum, item) => sum + item.paid, 0) / overallHistory.length, average_over_under: overallHistory.reduce((sum, item) => sum + item.over_under, 0) / overallHistory.length } : undefined
  ...
  const historicalPaidValues = [slot?.average_paid, overallSlots?.average_paid].filter((value): value is number => typeof value === 'number')
  const pastAveragePaid = historicalPaidValues.length ? historicalPaidValues.reduce((sum, value) => sum + value, 0) / historicalPaidValues.length : null
  ...
  return <div className="inspector-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section className="auction-inspector" style={{ transform: `translate(${dragOffset.x}px, ${dragOffset.y}px)` }} role="dialog" aria-modal="true" aria-labelledby="auction-inspector-title" onKeyDown={handleActionKeys}>
      ...
      <div className="auction-inspector__stats"><article><strong>{formatValue(row.sourceValue)}</strong><small>Base price</small></article><article><strong>{formatValue(row.adjustedValue)}</strong><small>Adjusted price</small></article><article><strong>{pastAveragePaid === null ? '—' : `$${pastAveragePaid.toFixed(1)}`}</strong><small>Past avg paid</small></article><article className={overUnder !== null && typeof overUnder === 'number' && overUnder > 1 ? 'is-over' : overUnder !== null && typeof overUnder === 'number' && overUnder < -1 ? 'is-under' : ''}><strong>{overUnder === null || overUnder === undefined ? '—' : formatSignedValue(Math.round(overUnder))}</strong><small>Historical O/U</small></article><article><strong>{draftedCount}</strong><small>{position} drafted</small></article></div>
      <details className="auction-inspector__history" open><summary><strong>Past years</strong><span>Expand</span></summary><div><HistoryTable label={`${position}${rank || '—'}`} history={yearHistory} summary={slot} /><HistoryTable label={`Overall ${overallRank || '—'}`} history={overallHistory} summary={overallSlots} /></div></details>
