...
import { enrichDraftRows, historicalPriceEstimate, overallSlotHistory, slotHistory, slotSummaries, type HistoricalYear } from '../data/draftAnalytics'
...
export function AuctionPlayerInspector({ row, allRows, dragOffset, rowsBySeason, season, teams, favorited, onFavorite, onClose, onDraft, onAdd, onNavigate, onDragOffsetChange }: Props) {
  ...
  const overallSlots = overallHistory.length ? { average_paid: overallHistory.reduce((sum, item) => sum + item.paid, 0) / overallHistory.length, average_expected: overallHistory.reduce((sum, item) => sum + item.expected, 0) / overallHistory.length, average_over_under: overallHistory.reduce((sum, item) => sum + item.over_under, 0) / overallHistory.length } : undefined
  const team = normalizeTeamAbbreviation(row.team)
  const positionDiff = slot?.average_over_under
  const overallDiff = overallSlots?.average_over_under
  const historicalSignal = [positionDiff, overallDiff].filter((value): value is number => typeof value === 'number' && Number.isFinite(value))
  const historicalSignalAverage = historicalSignal.length ? historicalSignal.reduce((sum, value) => sum + value, 0) / historicalSignal.length : null
  const historicalPrice = typeof row.sourceValue === 'number' ? historicalPriceEstimate(row.sourceValue, positionDiff, overallDiff) : row.sourceValue
