      ...
      {picks.length ? <ol className="recent-draft__list">
        {picks.slice(0, 5).map((row, index) => {
          ...
          return <li key={id} className={isMine ? 'is-mine' : ''}>
            <div className="recent-draft__meta"><span className="recent-draft__pick">{index === 0 ? 'Last pick' : `Pick ${picks.length - index}`}</span><span className={`recent-draft__pos pos-${row.positionTone ?? 'other'}`}>{row.position}</span><button className={`recent-draft__mine ${isMine ? 'active' : ''}`} type="button" aria-label={`${isMine ? 'Remove' : 'Add'} ${row.player} ${isMine ? 'from' : 'to'} my roster`} aria-pressed={isMine} onClick={() => onMine(id)}>{isMine ? '✓' : '+'}</button></div>
