...
type Props = { rows: RankingRow[]; drafted: Set<string>; picks: Record<string, number>; mine: Set<string>; teams: Record<string, TeamAsset>; mode: 'auction' | 'snake'; onMine: (id: string) => void; onUndraft: (id: string) => void; onPick: (id: string, value: number | undefined) => void; onReorder: (fromId: string, toId: string) => void }

/** A compact, chronological audit trail for draft picks. */
export function RecentDraftPanel({ rows, drafted, picks, mine, teams, mode, onMine, onUndraft, onPick, onReorder }: Props) {
