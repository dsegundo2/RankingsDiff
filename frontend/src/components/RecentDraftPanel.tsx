...
type Props = { rows: RankingRow[]; drafted: Set<string>; mine: Set<string>; teams: Record<string, TeamAsset>; onMine: (id: string) => void; isAuction?: boolean }

/** A compact, chronological audit trail for draft picks. */
export function RecentDraftPanel({ rows, drafted, mine, teams, onMine, isAuction = false }: Props) {
      ...
      {picks.length ? <ol className="recent-draft__list">
        {picks.slice(0, 5).map((row, index) => {
          ...
          return <li key={id} className={isMine ? 'is-mine' : ''}>
            <div className="recent-draft__meta"><span className="recent-draft__pick">{index === 0 ? 'Last pick' : isAuction ? 'Drafted' : `Pick ${picks.length - index}`}</span></div>
            <div className="recent-draft__player"><TeamBadge team={team} asset={getTeamAsset(teams, team)} /><span className="recent-draft__identity"><strong>{row.player}</strong><button className={`recent-draft__mine ${isMine ? 'active' : ''}`} type="button" aria-label={`${isMine ? 'Remove' : 'Add'} ${row.player} ${isMine ? 'from' : 'to'} my roster`} aria-pressed={isMine} onClick={() => onMine(id)}>{isMine ? '✓' : '+'}</button><small>{row.team} · {row.position}</small></span></div>
          </li>
        })}
      </ol> : null}
    </section>
