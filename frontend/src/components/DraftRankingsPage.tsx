...
type AverageSortKey = 'rank' | 'average_paid' | 'average_over_under' | 'highest_paid' | 'lowest_paid'
...
export function DraftRankingsPage({ season, rows, rowsBySeason, teams, onNavigate, onSeason }: Props) {
  ...
  const managerPatterns = useMemo(() => {
    ...
    return [...new Set(allDraftRows.map((row) => row.manager))].sort().map((manager) => {
      ...
      const highs = Object.fromEntries(POSITIONS.map((position) => { const row = [...managerRows].filter((candidate) => candidate.position.toUpperCase() === position).sort((left, right) => right.offer_amount - left.offer_amount)[0]; return [position, row] })) as Record<string, DraftRankingRow | undefined>
  ...
  return <main className="dashboard draft-rankings-page">
    ...
    <section className="historic-averages" aria-label={`${averageLabel} historic average prices`}><div className="historic-averages__intro"><div><span className="eyebrow">Historic average · 2022–2025</span><h2>League average prices</h2><p>Compare draft slots across seasons, then sort by the baseline or price range.</p></div><span className="historic-averages__meta">Tier size {6} · sample 4 seasons</span></div><div className="table-wrap historic-averages__table-wrap"><table className="rankings-table historic-averages__table"><thead><tr><th>Slot</th>{([['average_paid', 'Avg paid'], ['average_over_under', 'Over / under'], ['highest_paid', 'Highest paid'], ['lowest_paid', 'Lowest paid']] as const).map(([key, label], index) => <Fragment key={key}><th className={`num historic-sortable ${averageSortKey === key ? 'is-sorted' : ''}`} role="button" tabIndex={0} aria-sort={averageSortKey === key ? averageSortDirection === 'asc' ? 'ascending' : 'descending' : 'none'} onClick={() => sortAveragesBy(key)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); sortAveragesBy(key) } }}>{label}{averageSortKey === key ? ` ${averageSortDirection === 'asc' ? '↑' : '↓'}` : ''}</th>{index === 0 ? <th className="num">Avg expected</th> : null}</Fragment>)}</tr></thead><tbody>{sortedHistoricAverages.map((summary) => <tr key={`${summary.position}-${summary.rank}`}><td><strong>{summary.position === 'OVERALL' ? `Overall ${summary.rank}` : `${summary.position}${summary.rank}`}</strong></td><td className="num"><strong>{averageMoney(summary.average_paid)}</strong></td><td className="num"><strong>{averageMoney(summary.average_expected)}</strong></td><td className={`num ${summary.average_over_under !== null && summary.average_over_under >= 0 ? 'is-over' : 'is-under'}`}><strong>{averageDelta(summary.average_over_under)}</strong></td><td className="num">{averageMoney(summary.highest_paid)}</td><td className="num">{averageMoney(summary.lowest_paid)}</td></tr>)}</tbody></table></div></section>
    <section className="historic-patterns" aria-label="Year over year auction patterns"><div className="historic-section-heading"><div><span className="eyebrow">Year over year · all positions</span><h2>How the room is moving</h2></div><span>Average paid per player and the change from the prior draft.</span></div><div className="table-wrap historic-patterns__table-wrap"><table className="rankings-table historic-patterns__table historic-year-table"><thead><tr><th>Season</th><th className="num">Avg paid / player</th><th className="num">Change</th><th>Biggest purchase</th></tr></thead><tbody>{yearPatterns.map((pattern) => <tr key={pattern.year}><td><strong>{pattern.year}</strong></td><td className="num"><strong>{averageMoney(pattern.average)}</strong></td><td className={`num ${pattern.change !== null && pattern.change >= 0 ? 'is-over' : 'is-under'}`}><strong>{averageDelta(pattern.change)}</strong></td><td>{pattern.biggest ? `${pattern.biggest.player} · ${moneyTenth(pattern.biggest.offer_amount)}` : '—'}</td></tr>)}</tbody></table></div></section>
    <section className="historic-patterns" aria-label="All-time historic records"><div className="historic-section-heading"><div><span className="eyebrow">All-time records · 2022–2025</span><h2>Highest purchases ever</h2></div><span>Across every imported auction, not just the selected season.</span></div><div className="historic-records__grid">{allTimeRecords.map(({ key, row }) => <article key={key}><span>{key === 'ALL' ? 'Highest purchase' : `Highest ${key} purchase`}</span><strong>{row ? moneyTenth(row.offer_amount) : '—'}</strong><small>{row ? `${row.player} · ${row.manager} · ${row.season}` : 'No result'}</small></article>)}</div></section>
    ...
    <section className="historic-patterns" aria-label="Position market patterns"><div className="historic-section-heading"><div><span className="eyebrow">Overall league patterns · 2022–2025</span><h2>Position market</h2></div><span>Use the average as a baseline; watch the ceiling and share of spend.</span></div><div className="table-wrap historic-patterns__table-wrap"><table className="rankings-table historic-patterns__table historic-position-table"><thead><tr><th>Position</th><th className="num">Avg paid / player</th><th className="num">Median</th><th className="num">Highest purchase</th><th>Player · manager</th><th className="num">Variance</th><th className="num">Share of spend</th></tr></thead><tbody>{positionPatterns.map((pattern) => <tr key={pattern.position}><td><span className={`pos-chip pos-${pattern.position.toLowerCase()}`}>{pattern.position}</span></td><td className="num"><strong>{moneyTenth(pattern.average)}</strong></td><td className="num">{moneyTenth(pattern.median)}</td><td className="num">{pattern.highest ? moneyTenth(pattern.highest.offer_amount) : '—'}</td><td>{pattern.highest ? `${pattern.highest.player} · ${pattern.highest.manager}` : '—'}</td><td className={`num ${pattern.variance >= 0 ? 'is-over' : 'is-under'}`}>{deltaTenth(pattern.variance)}</td><td className="num">{(pattern.share * 100).toFixed(1)}%</td></tr>)}</tbody></table></div></section>
