...
export function DraftRankingsPage({ season, rows, rowsBySeason, teams, onNavigate, onSeason }: Props) {
  ...
  return <main className="dashboard draft-rankings-page">
    ...
    {view === 'averages' ? <section className="historic-averages" aria-label="Historical auction price signals">
      ...
      <div className="table-wrap historic-averages__table-wrap"><table className="rankings-table historic-averages__table"><thead><tr><th>Slot</th><th>Avg paid</th><th>Avg expected</th><th>Over / under</th><th>Sample</th><th>Tier avg</th><th>Tier O/U</th><th>Seasons 2022–2025</th></tr></thead><tbody>{slotRows.slice(0, HISTORIC_LIMITS[averagePosition]).map((summary, index) => { const tier = tierRows.find((item) => summary.rank >= item.start_rank && summary.rank <= item.end_rank); const tierBreak = Boolean(tier && (index === 0 || tier.rank !== tierRows.find((item) => slotRows[index - 1].rank >= item.start_rank && slotRows[index - 1].rank <= item.end_rank)?.rank)); return <tr className={tierBreak ? "historic-tier-start" : undefined} key={summary.rank}><td><strong>{averagePosition}{summary.rank}</strong></td><td className="num"><strong>{summary.average_paid === null ? '—' : money(Math.round(summary.average_paid))}</strong></td><td className="num">{summary.average_expected === null ? '—' : money(Math.round(summary.average_expected))}</td><td className={`num draft-value-diff ${summary.average_over_under && summary.average_over_under > 1 ? 'is-over' : summary.average_over_under && summary.average_over_under < -1 ? 'is-under' : ''}`}>{summary.average_over_under === null ? '—' : delta(Math.round(summary.average_over_under))}</td><td className="num">{summary.sample_size}</td><td className="num">{tier?.average_paid === null || !tier ? '—' : money(Math.round(tier.average_paid))}</td><td className={`num draft-value-diff ${tier?.average_over_under && tier.average_over_under > 1 ? 'is-over' : tier?.average_over_under && tier.average_over_under < -1 ? 'is-under' : ''}`}>{tier?.average_over_under == null ? '—' : delta(Math.round(tier.average_over_under))}</td><td className="num">2022–2025</td></tr> })}</tbody></table></div>
    </section> : <>
    ...
    <div className="table-wrap draft-rankings-table-wrap"><table className="rankings-table draft-rankings-table"><thead><tr><th><button className="sort-button" onClick={() => sortBy('rank')}>Rank{sortKey === 'rank' ? ` ${sortDirection === 'asc' ? '↑' : '↓'}` : ''}</button></th><th><button className="sort-button" onClick={() => sortBy('player')}>Player{sortKey === 'player' ? ` ${sortDirection === 'asc' ? '↑' : '↓'}` : ''}</button></th><th><button className="sort-button" onClick={() => sortBy('position')}>Pos{sortKey === 'position' ? ` ${sortDirection === 'asc' ? '↑' : '↓'}` : ''}</button></th><th><button className="sort-button" onClick={() => sortBy('manager')}>Manager{sortKey === 'manager' ? ` ${sortDirection === 'asc' ? '↑' : '↓'}` : ''}</button></th><th><button className="sort-button" onClick={() => sortBy('offer_amount')}>Paid{sortKey === 'offer_amount' ? ` ${sortDirection === 'asc' ? '↑' : '↓'}` : ''}</button></th><th><button className="sort-button" onClick={() => sortBy('espn_suggested_value')}>ESPN value / expected{sortKey === 'espn_suggested_value' ? ` ${sortDirection === 'asc' ? '↑' : '↓'}` : ''}</button></th><th><button className="sort-button" onClick={() => sortBy('value_diff')}>Over / under{sortKey === 'value_diff' ? ` ${sortDirection === 'asc' ? '↑' : '↓'}` : ''}</button></th><th>Slot</th><th>Tier</th></tr></thead><tbody>
      {filteredRows.map((row, index) => <>{index > 0 && index % DRAFT_SIZE === 0 ? <tr className="draft-rankings-divider" key={`divider-${index}`}><td colSpan={9}><span>Top {index}</span></td></tr> : null}<tr key={`${row.player}|${row.nfl_team}`}><td className="num">{row.rank}</td><td><div className="draft-player-cell"><TeamBadge team={normalizeTeamAbbreviation(row.nfl_team)} asset={getTeamAsset(teams, normalizeTeamAbbreviation(row.nfl_team))} /><span><strong>{row.player}</strong><small className="draft-rankings-team">{row.nfl_team}</small></span></div></td><td><span className={`pos-chip pos-${row.position.toLowerCase().replace('/', '-')}`}>{row.position}</span></td><td>{row.manager}</td><td className="num"><strong>{money(row.offer_amount)}</strong></td><td className="num">{money(row.espn_suggested_value)}</td><td className={`num draft-value-diff ${row.value_diff > 1 ? 'is-over' : row.value_diff < -1 ? 'is-under' : ''}`}><strong>{delta(row.value_diff)}</strong></td><td className="num"><strong>{row.position_rank ? `${row.position}${row.position_rank}` : '—'}</strong></td><td>Tier {row.tier}</td></tr></>)}
