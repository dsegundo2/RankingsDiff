...
export function DraftRankingsPage({ season, rows, rowsBySeason, teams, onNavigate, onSeason }: Props) {
  ...
  const managerPatterns = useMemo(() => {
    return [...new Set(allDraftRows.map((row) => row.manager))].sort().map((manager) => {
      ...
      return { manager, biggest, highs }
    })
  }, [allDraftRows])
  const positionSlotPatterns = useMemo(() => {
    ...
    const historicalSlotRows = HISTORICAL_SEASONS.flatMap((year) => enrichDraftRows(rowsBySeason[year] ?? [], DRAFT_SIZE).map((row) => ({ ...row, season: year })))
    return POSITION_MARKET_SLOTS.map(([position, rank]) => {
      const values = historicalSlotRows.filter((row) => row.position.toUpperCase() === position && row.position_rank === rank)
      ...
      const slotSpend = values.reduce((sum, row) => sum + row.offer_amount, 0)
      return { slot: `${position}${rank}`, position, median: median(values.map((row) => row.offer_amount)), highest, variance: average - leagueAverage, share: totalSpend ? slotSpend / totalSpend : 0 }
    })
  ...
  return <main className="dashboard draft-rankings-page">
    ...
    <section className="historic-patterns" aria-label="Position market patterns"><div className="historic-section-heading"><div><span className="eyebrow">Position market · 2022–2025</span><h2>Position market</h2></div><span>Each row tracks the first one or two players drafted at that position.</span></div><div className="table-wrap historic-patterns__table-wrap"><table className="rankings-table historic-patterns__table historic-position-table"><thead><tr><th>Slot</th><th className="num">Median result</th><th className="num">Highest purchase</th><th>Player · manager</th><th className="num">Vs league</th><th className="num">Share of spend</th></tr></thead><tbody>{positionSlotPatterns.map((pattern) => <tr key={pattern.slot}><td><span className={`pos-chip pos-${pattern.position.toLowerCase()}`}>{pattern.slot}</span></td><td className="num"><strong>{moneyTenth(pattern.median)}</strong></td><td className="num">{pattern.highest ? moneyTenth(pattern.highest.offer_amount) : '—'}</td><td><HistoricPlayer row={pattern.highest} teams={teams} detail={pattern.highest?.manager} /></td><td className={`num ${pattern.variance >= 0 ? 'is-over' : 'is-under'}`}>{deltaTenth(pattern.variance)}</td><td className="num">{(pattern.share * 100).toFixed(1)}%</td></tr>)}</tbody></table></div></section>
    <section className="historic-patterns" aria-label="Manager position highs"><div className="historic-section-heading"><div><span className="eyebrow">Manager patterns · all years</span><h2>Highest paid player by position</h2></div><span>Ties are shown when managers paid the same top price.</span></div><div className="table-wrap historic-patterns__table-wrap"><table className="rankings-table historic-patterns__table historic-position-table historic-manager-highs-table"><thead><tr><th>Manager</th>{POSITIONS.map((position) => <th key={position}>{position}</th>)}</tr></thead><tbody>{managerPatterns.map((pattern) => <tr key={pattern.manager}><td><strong>{pattern.manager}</strong></td>{POSITIONS.map((position) => <td key={position}>{pattern.highs[position].length ? <div className="historic-player-ties">{pattern.highs[position].map((row) => <HistoricPlayer key={`${row.player}-${row.offer_amount}`} row={row} teams={teams} detail={moneyTenth(row.offer_amount)} />)}</div> : '—'}</td>)}</tr>)}</tbody></table></div></section>
