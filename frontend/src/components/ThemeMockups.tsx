...
export function ThemeMockups() {
  ...
  return <main className={`theme-lab theme-lab--${selected}`}>
    <div className="theme-lab__topbar"><a className="theme-lab__back" href="./">← Back to dashboard</a><span className="mock-eyebrow">Draft Distillery · Theme lab</span><span className="theme-lab__status">Responsive mockups</span></div>
    <header className="theme-lab__intro"><div><span className="mock-eyebrow">A visual direction study</span><h1>Choose a calmer, more connected header.</h1><p>Six responsive HTML header mockups using the existing Draft Distillery language. Each option stays compact so the rankings remain the main event.</p></div><div className="theme-lab__choice"><span>Selected direction</span><strong>{activeTheme.name}</strong><button className="mock-primary" type="button" onClick={() => setSettingsOpen((open) => !open)}>{settingsOpen ? 'Hide settings' : 'Preview settings'}</button></div></header>
