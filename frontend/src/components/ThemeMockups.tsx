...
const themes: Array<{ id: ThemeId; name: string; eyebrow: string; description: string }> = [
  { id: 'material', name: 'Material Calm', eyebrow: 'Option 01', description: 'Tonal surfaces, clear hierarchy, and friendly blue-violet actions.' },
  { id: 'command', name: 'Draft Command', eyebrow: 'Option 02', description: 'Dense, high-signal tooling for managers who live in the table.' },
  { id: 'midnight', name: 'Midnight Focus', eyebrow: 'Option 03', description: 'A dark, low-glare workspace with bright data accents.' },
]
...
export function ThemeMockups() {
  ...
  return <main className={`theme-lab theme-lab--${selected}`}>
    ...
    <section className="theme-options" aria-label="Theme options">{themes.map((theme) => <button type="button" key={theme.id} className={`theme-option${theme.id === selected ? ' is-selected' : ''}`} onClick={() => setSelected(theme.id)}><span className="theme-option__swatches"><i /><i /><i /></span><span className="mock-eyebrow">{theme.eyebrow}</span><strong>{theme.name}</strong><small>{theme.description}</small><span className="theme-option__check" aria-hidden="true">{theme.id === selected ? '✓' : '○'}</span></button>)}</section>
