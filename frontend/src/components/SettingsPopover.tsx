          ...
          <div className="settings-content">
            ...
            {activePane === 'display' ? <section className="settings-section settings-section--tools">
              ...
              <div className="settings-preference-list">
                ...
                <label className="settings-preference-card"><span><strong>My draft spot</strong><small>Show your current and future snake picks across 17 rounds.</small></span><span className="draft-slot-input"><input aria-label="My draft spot" type="number" min="1" max={draftSize} step="1" value={draftSlot} onChange={(event) => onDraftSlot(Math.min(draftSize, Math.max(1, Number(event.target.value) || 1)))} /><small>/ {draftSize}</small></span></label>
                <label className="settings-preference-card"><span><strong>Draft size</strong><small>Set the number of teams in the snake draft.</small></span><span className="draft-slot-input"><input aria-label="Draft size" type="number" min="2" max="20" step="1" value={draftSize} onChange={(event) => onDraftSize(Math.min(20, Math.max(2, Number(event.target.value) || 2)))} /><small>teams</small></span></label>
