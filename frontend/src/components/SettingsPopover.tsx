...
type Props = {
  ...
  onYahooProjectionColumn: (value: YahooProjectionColumn) => void
  onShowRegressionDiff: (value: boolean) => void
  onTargetQueueView: (value: TargetQueueView) => void
          ...
          <div className="settings-content">
            ...
            {activePane === 'display' ? <section className="settings-section settings-section--tools">
              <div className="settings-preference-list">
                ...
                <div className="settings-preference-card"><span><strong>Shortlist grouping</strong><small>Organize shortlisted players by position or keep one ranked list.</small></span><span className="settings-segmented" role="group" aria-label="Shortlist grouping"><button type="button" className={targetQueueView === 'normal' ? 'is-active' : ''} aria-pressed={targetQueueView === 'normal'} onClick={() => onTargetQueueView('normal')}>Normal</button><button type="button" className={targetQueueView === 'position' ? 'is-active' : ''} aria-pressed={targetQueueView === 'position'} onClick={() => onTargetQueueView('position')}>Position</button></span></div>
                ...
                <div className="settings-preference-card"><span><strong>Yahoo projections</strong><small>Choose which projected-points snapshot appears in the board column.</small></span><span className="settings-preference-card__controls"><select aria-label="Yahoo projection column" value={yahooProjectionColumn} onChange={(event) => onYahooProjectionColumn(event.target.value as YahooProjectionColumn)}><option value="season">Season proj</option><option value="week1">Week 1 proj</option><option value="both">Both projections</option></select><span className="drafted-toggle"><input type="checkbox" aria-label="Show Yahoo projections" checked={showYahooProjections} onChange={(event) => onShowYahooProjections(event.target.checked)} /></span></span></div>
